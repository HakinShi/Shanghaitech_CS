...
class RegressionModel(object):
    ...
    def __init__(self):
        ...
        self.w1 = nn.Parameter(self.input,self.m)
        self.b1 = nn.Parameter(1, self.m)
        self.w2 = nn.Parameter(self.m, self.output)
    ...
    def run(self, x):
        ...
        xw1b1xw2 = nn.Linear(nn.ReLU(xw1b1), self.w2)
        return nn.AddBias(xw1b1xw2, self.b2)
    ...
    def train(self, dataset):
        ...
        while jud > 0.02:
            for x, y in dataset.iterate_once(self.batch_size):
                ...
                self.w1.update(g_W1, -self.learningRate)
                self.b1.update(g_B1, -self.learningRate)
                self.w2.update(g_W2, -self.learningRate)
                self.b2.update(g_B2, -self.learningRate)
                jud = nn.as_scalar(loss)
    ...
    def train(self, dataset):
        ...
        while dataset.get_validation_accuracy() < .98:
            for x, y in dataset.iterate_once(self.batch_size):
                ...
                gradW1, gradB1, gradW2, gradB2, gradW3, gradB3 = nn.gradients(loss, [self.w1, self.b1, self.w2, self.b2, self.w3, self.b3])
                ...
                self.w3.update(gradW3, -self.learning_rate)
                self.b3.update(gradB3, -self.learning_rate)
...
class LanguageIDModel(object):
    ...
    def run(self, xs):
        ...
        for i in range(1, len(xs)):
                h = nn.ReLU(nn.Add(nn.Linear(xs[i], self.w1), nn.Linear(h, self.w2)))
    ...
    def train(self, dataset):
        ...
        while dataset.get_validation_accuracy() < 0.85:
            for x, y in dataset.iterate_once(self.batch_size):
                loss = self.get_loss(x, y)
                g_W, g_W2, g_W3 = nn.gradients(loss, [self.w1, self.w2, self.w3])
                self.w1.update(g_W, -self.learning_rate)
